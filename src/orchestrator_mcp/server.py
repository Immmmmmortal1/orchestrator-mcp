...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "or per-run overrides. Current build uses stub providers only (no real LLM calls). "
...
def orchestrate_run_start(
    ...
    try:
        ...
        return _json(
            _engine.start_run(goal=goal, profile_name=resolved, stage_overrides=overrides)
        )
...
def orchestrate_run_pipeline(
    ...
    try:
        ...
        resolved = resolve_profile_name(profile)
        return _json(
            _engine.run_pipeline(goal=goal, profile_name=resolved, stage_overrides=overrides)
        )
    except PipelineError as exc:
        return _json({"ok": False, "error": str(exc)})
    except Exception as exc:
